...
export async function bulkPostFactory ({ url, output }) {
  ...
  function onClose ({ quiet = false }) {
    if (errorsFile && !quiet) {
      ...
      if (successesFile) process.stderr.write(grey(`Successful updates responses saved to ${successesFile}`) + successesCountColorFn(` ${successesCount} doc(s) successfully saved`)  + '\n')
